...
@Component
public class DefaultCategoryValidator implements CategoryValidatorStrategy {

    private static final Set<String> DEFAULT_ATTRIBUTES = Set.of(
            "MATERIAL", "CAPACIDAD", "TAMAÑO","USO"
    );
    ...
    public void validateCategory(List<AttributeRequestDto> attributeRequestDtos) {
        if(attributeRequestDtos == null || attributeRequestDtos.isEmpty()){
            throw new InvalidAttributeException(Messages.CATEGORY_DEFAULT_MESSAGE);
        }

        boolean hasRequiredAttributes = attributeRequestDtos.stream()
                .map(attr -> attr.attributeName().toUpperCase())
                .anyMatch(DEFAULT_ATTRIBUTES::contains);

        if(!hasRequiredAttributes){
            throw new InvalidAttributeException(Messages.INVALID_DEFAULT_DEPORTS_MESSAGE+DEFAULT_ATTRIBUTES);
        }
    }
}
