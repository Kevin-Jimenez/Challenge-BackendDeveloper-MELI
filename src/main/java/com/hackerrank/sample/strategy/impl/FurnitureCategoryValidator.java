...
@Component
public class FurnitureCategoryValidator implements CategoryValidatorStrategy {

    private static final Set<String> FURNITURE_ATTRIBUTES = Set.of(
            "MATERIAL", "CAPACIDAD", "TAMAÑO",
            "USO"
    );
    ...
    public void validateCategory(List<AttributeRequestDto> attributeRequestDtos) {
        if(attributeRequestDtos == null || attributeRequestDtos.isEmpty()){
            throw new InvalidAttributeException(Messages.CATEGORY_FURNITURE_MESSAGE);
        }

        boolean hasRequiredAttributes = attributeRequestDtos.stream()
                .map(attr -> attr.attributeName().toUpperCase())
                .anyMatch(FURNITURE_ATTRIBUTES::contains);

        if(!hasRequiredAttributes){
            throw new InvalidAttributeException(Messages.INVALID_CATEGORY_FURNITURE_MESSAGE+FURNITURE_ATTRIBUTES);
        }
    }
}
